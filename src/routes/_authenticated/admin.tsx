...
export const Route = createFileRoute("/_authenticated/admin")({
  beforeLoad: async ({ context }) => {
    // Admin guard — checked via JWT role (set at login time)
...
function AdminPage() {
  ...
  const fetchComplaints = useServerFn(getMyComplaints); // admin sees all via getMyComplaints
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      {analytics && analytics.slaBreaches > 0 && (
        <div className="flex items-center gap-2 rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">
          <span className="font-semibold">{analytics.slaBreaches}</span>
          {analytics.slaBreaches === 1 ? " ticket has" : " tickets have"} breached the 48-hour SLA.
        </div>
      )}
          ...
          <CardContent className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={analytics?.trend ?? []}>
                ...
                <Tooltip
                  contentStyle={{ borderRadius: 8, border: "1px solid oklch(0.9 0.01 260)" }}
                />
                <Line
                  type="monotone"
                  dataKey="count"
            ...
            <div className="divide-y">
              {all.map((c) => (
                <Link
                  ...
                  to="/complaints/$id"
                  params={{ id: c.id }}
                  className="flex flex-col gap-2 py-3 transition hover:bg-accent/40 sm:flex-row sm:items-center sm:justify-between"
                >
